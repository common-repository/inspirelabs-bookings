...
use BookingsVendor\PhpConsole\Connector;
use BookingsVendor\PhpConsole\Handler;
use BookingsVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \BookingsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \BookingsVendor\PhpConsole\Connector $connector = null, $level = \BookingsVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('BookingsVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\BookingsVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \BookingsVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \BookingsVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\BookingsVendor\PhpConsole\Helper::isRegistered()) {
            \BookingsVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \BookingsVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \BookingsVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \BookingsVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \BookingsVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
