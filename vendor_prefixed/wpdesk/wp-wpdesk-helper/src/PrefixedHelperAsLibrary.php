...
namespace BookingsVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use BookingsVendor\WPDesk\Helper\Debug\LibraryDebug;
use BookingsVendor\WPDesk\Helper\Integration\LicenseIntegration;
use BookingsVendor\WPDesk\Helper\Integration\LogsIntegration;
use BookingsVendor\WPDesk\Helper\Integration\SettingsIntegration;
use BookingsVendor\WPDesk\Helper\Integration\TrackerIntegration;
use BookingsVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use BookingsVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \BookingsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    const LIBRARY_TEXT_DOMAIN = 'inspirelabs-bookings';
    ...
    {
        $helper_info = new \BookingsVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        //$subscription_integration = new \BookingsVendor\WPDesk\Helper\Integration\LicenseIntegration();
        //$subscription_integration->hooks();
        $settingsPage = new \BookingsVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \BookingsVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        //$tracker_integration = new \BookingsVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \BookingsVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \BookingsVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \BookingsVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \BookingsVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \BookingsVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
