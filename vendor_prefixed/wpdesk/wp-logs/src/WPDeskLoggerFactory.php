...
use LogicException;
use BookingsVendor\Monolog\Handler\NullHandler;
use BookingsVendor\Monolog\Logger;
use BookingsVendor\Monolog\Registry;
use BookingsVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use BookingsVendor\WPDesk\Logger\WC\WooCommerceCapture;
use BookingsVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \BookingsVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\BookingsVendor\Monolog\Registry::hasLogger($name)) {
            \BookingsVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\BookingsVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \BookingsVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \BookingsVendor\Monolog\Logger($name);
        }
        if (\BookingsVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \BookingsVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\BookingsVendor\Monolog\Logger $logger)
    {
        if (!\defined('BookingsVendor\\WC_LOG_THRESHOLD')) {
            \define('BookingsVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \BookingsVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \BookingsVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \BookingsVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \BookingsVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\BookingsVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \BookingsVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \BookingsVendor\Monolog\Registry::hasLogger($name);
    }
}
