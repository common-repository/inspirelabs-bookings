...
namespace BookingsVendor\WPDesk\Plugin\Flow\Initialization\Simple;

use BookingsVendor\WPDesk\Helper\HelperRemover;
use BookingsVendor\WPDesk\Helper\PrefixedHelperAsLibrary;
use BookingsVendor\WPDesk\License\PluginRegistrator;
use BookingsVendor\WPDesk\Plugin\Flow\Initialization\ActivationTrait;
use BookingsVendor\WPDesk\Plugin\Flow\Initialization\BuilderTrait;
use BookingsVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait;
use BookingsVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy;
use BookingsVendor\WPDesk\PluginBuilder\Plugin\ActivationAware;
 ...
 */
class SimplePaidStrategy implements \BookingsVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    /** @var SlimPlugin */
    private $plugin;
    public function __construct(\BookingsVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_before_init(\BookingsVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_init(\BookingsVendor\WPDesk_Plugin_Info $plugin_info)
    {
        if (!$this->plugin) {
            $this->plugin = $this->build_plugin($plugin_info);
        }
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \BookingsVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \BookingsVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \BookingsVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \BookingsVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \BookingsVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
