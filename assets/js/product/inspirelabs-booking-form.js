class BookingForm {
	...
	calculatePrice(reservation = false) {
        ...
        if (reservation) {
            ...
            let liCheckIn = document.createElement("LI")
                liCheckIn.appendChild(document.createTextNode(booking.alerttext.from + ': ')); //Chek-in
			...
			Object.keys(reservation).forEach( (day, index) => {
				...
				// !!! do not show price for last day
                //if ( index != ( Object.keys(reservation).length - 1)) {
                if ( index != ( Object.keys(reservation).length )) {
            ...
            let liCheckOut = document.createElement("LI");
                liCheckOut.appendChild(document.createTextNode(booking.alerttext.to + ': ')); // Chek out
    ...
    addShortBookingFees(duration, total) {
        ...
        if( booking.pro_mode ) {
            if (duration < 4 && !jQuery.isEmptyObject(fees)) {
                let fee = +fees[duration];
